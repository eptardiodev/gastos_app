...
class AppDatabase {
  ...
  _initDb() async {
    String documentsDirectory = Platform.isAndroid
  ...
  Future _insertInitialData(Database db) async {
    ...
    await db.insert('categories',
        {'name': 'Transporte', 'icon_name': 'bus', 'color_hex': '#4ECDC4'});
    await db.insert('categories',
        {'name': 'Misceláneo', 'icon_name': 'box', 'color_hex': '#8395A7'});

    // Insert subcategories
    final alimentosId = (await db
            .query('categories', where: 'name = ?', whereArgs: ['Alimentos']))
        .first['id'] as int;
    final transporteId = (await db
            .query('categories', where: 'name = ?', whereArgs: ['Transporte']))
        .first['id'] as int;

    await db.insert(
        'subcategories', {'category_id': alimentosId, 'name': 'Panadería'});
    await db.insert(
        'subcategories', {'category_id': alimentosId, 'name': 'Carnes'});
    await db.insert(
        'subcategories', {'category_id': transporteId, 'name': 'Combustible'});

    // Insert products
    final panaderiaId = (await db.query('subcategories',
            where: 'name = ?', whereArgs: ['Panadería']))
        .first['id'] as int;
    final carnesId = (await db
            .query('subcategories', where: 'name = ?', whereArgs: ['Carnes']))
        .first['id'] as int;
    final combustibleId = (await db.query('subcategories',
            where: 'name = ?', whereArgs: ['Combustible']))
        .first['id'] as int;
    ...
    final groupId = (await db
            .query('groups', where: 'name = ?', whereArgs: ['Casa Familiar']))
        .first['id'] as int;
    ...
    // Insert list items
    final panBlancoId = (await db
            .query('products', where: 'name = ?', whereArgs: ['Pan blanco']))
        .first['id'] as int;
    final carneMolidaId = (await db
            .query('products', where: 'name = ?', whereArgs: ['Carne molida']))
        .first['id'] as int;
    final salchichasId = (await db
            .query('products', where: 'name = ?', whereArgs: ['Salchichas']))
        .first['id'] as int;

    final unidadId = (await db.query('measurement_units',
            where: 'name = ?', whereArgs: ['Unidad']))
        .first['id'] as int;
    final kgId = (await db.query('measurement_units',
            where: 'name = ?', whereArgs: ['Kilogramo']))
        .first['id'] as int;
    final paqueteId = (await db.query('measurement_units',
            where: 'name = ?', whereArgs: ['Paquete']))
        .first['id'] as int;
    ...
    await db.insert('transactions', {
      'user_id': '12',
      'product_id': (await db
              .query('products', where: 'name = ?', whereArgs: ['Gasolina 95']))
          .first['id'],
      'amount': 45.30,
      'quantity': 10,
      'price': 4.53,
      'unit_id': (await db.query('measurement_units',
              where: 'name = ?', whereArgs: ['Litro']))
          .first['id'],
